...
Windows 严格验证用例位于 [`docs/verification/windows`](docs/verification/windows)。目录采用“一份 Markdown 对应一个独立测试”的组织方式，文件名中的 `WIN-xxx` 是稳定用例编号。

修改网络采集、公网验证、target 探测、事件、调度或生命周期代码时：

1. 根据文件名选择所有受影响的验证用例，不要只执行直接修改文件对应的单个场景。
2. 严格按照用例中的输入条件、执行步骤和通过要求测试，并留存指定证据。
3. 标记为“预期失败/待修复”的反例不得被当作通过项，也不得为了适配当前实现而降低通过标准。
4. 如果代码改变了公开行为，应先更新设计文档和对应验证用例，再接受新的测试结论。
