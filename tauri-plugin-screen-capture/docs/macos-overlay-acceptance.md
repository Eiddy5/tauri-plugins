...
| 无关窗口移动 | 拖动或缩放窗口 B，但不改变 A | A 的浮层不因 B 的鼠标拖动而整体隐藏；实际遮住的绿色像素仍由 Window Server 自然遮挡 |
| 目标内部聚焦 | 从其他应用切回并点击 A 的内容区域 | 浮层在应用激活事务完成后的下一轮主 RunLoop 恢复正确相对层级，不先掉到 A 下方再等待低频纠偏 |
| 同应用兄弟窗口 | 用 A 所属应用的另一个窗口覆盖 A | 兄弟窗口自然遮住浮层，行为与跨进程窗口一致 |
| 透明角区域 | 同应用辅助 surface 只覆盖某个 32×32 panel 的透明内部 | 只要没有覆盖实际 4 point 绿色 L 形线段，该角仍显示；不得误隐藏左上角 |
| 拖动 | 按住并拖动 A | frame 变化后的一个 100 ms 采样周期内隐藏；拖动停止并得到首个稳定样本后按最新边界恢复 |
| 缩放 | 连续拖动 A 的任一窗口边缘 | 缩放期间保持隐藏；尺寸稳定后按新边界恢复，不出现旧尺寸角标 |
| 程序化移动 | 用应用命令或脚本改变 A 的位置/尺寸 | 行为与鼠标拖动一致，不依赖鼠标事件或前台焦点 |
...
| 暂停/恢复 | 暂停再恢复共享 | 暂停时 panel、100 ms 位置 timer 与 2 秒纠偏 timer 停止；恢复先刷新边界再显示 |
| 停止共享 | 停止会话 | 四个 panel、notification observer 和两个 timer 全部释放 |
...
| 显示器 | 采集开始后新建一个普通 Tauri 窗口 | 最迟一个低频刷新周期（约 2 秒）后，新窗口出现在共享画面；期间所有浮层始终不进入画面 |
...
- 100 ms 位置采样只查询目标 `CGWindowID`；frame 未变化时不读取完整窗口列表，不调用 `setFrame` 或 `orderWindow_relativeTo`。
- 2 秒纠偏快照未发现 frame、layer 或相对顺序变化时，不调用 `setFrame` 或 `orderWindow_relativeTo`。
- 从目标 frame 实际变化到四个 panel 隐藏的 P95 小于 `150 ms`。
- 从目标 frame 最后一次变化到四个 panel 恢复可见的 P95 小于 `150 ms`。
