...
窗口 source ID 包含 `CGWindowID`。窗口边界、可见性、`kCGWindowLayer` 和全局前后顺序通过 `CGWindowListCopyWindowInfo` 获取，并组成同一个目标快照。Quartz 窗口坐标必须按显示器转换成 AppKit screen coordinate，不能假设主显示器原点或统一缩放比例。测试需要覆盖位于主显示器上、下、左、右的显示器，以及 Retina 混合缩放布局。
...
2. 监听鼠标左键释放事件。只重新读取一次目标边界和 Z-order，在同一个主线程事务中重定位 4 个 panel、同步目标 layer，并重新建立相对排序；如果目标仍符合条件，则重新显示。
3. 监听 `NSWorkspace` 的应用激活、隐藏、终止、活动 Space、睡眠和唤醒通知。目标应用或其他应用激活时重新检查 Z-order；目标被其他窗口盖住时不提升 panel，目标重新到前台时恢复相对排序。
4. 监听显示器配置变化，并重新计算显示器边界。
5. 窗口 overlay 可见期间运行低频校正检查，以捕获程序化移动、程序化 reorder 或遗漏的通知。边界、layer 和相对顺序均未变化时，不提交任何 AppKit 更新。
...
src/overlay/
  ...
  macos/
    ...
    window.rs                    NSPanel/NSView/CALayer 创建、层级和更新
    bounds.rs                    source ID、CGWindow/NSScreen 边界、层级和坐标
...
macOS 没有 Win32 实现所使用的 owned-window 关系，因此不能依赖系统自动绑定生命周期和层级。实现通过同步 `kCGWindowLayer` 并使用 `order(.above, relativeTo: target_window_id)` 建立相邻顺序，使更靠前的窗口自然遮挡 overlay。每次相关激活、鼠标释放、Space 变化或低频快照发现 reorder 时重新验证该关系。若验证失败则隐藏 overlay，绝不回退为永久 topmost。最终接受该策略前，必须通过真实跨进程窗口测试。
